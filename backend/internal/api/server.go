...
type DataReader interface {
	GetAll(ctx context.Context, from, to int) (*database.DataResponse, error)
}
...
func (s *Server) handleGetData(w http.ResponseWriter, r *http.Request) {
	from, to, err := parseGetDataInput(r)
	if err != nil { http.Error(w, err.Error(), http.StatusBadRequest); return }
	data, dataErr := s.data.GetAll(r.Context(), from, to)
	writeJSON(w, data, dataErr)
}

// parseGetDataInput extracts and validates the from/to query parameters.
// Both must be positive integers less than 1 billion, with to >= from.
func parseGetDataInput(r *http.Request) (int, int, error) {
